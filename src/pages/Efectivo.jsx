...
const Efectivo = () => {
  ...
  const handleCountChange = (denomination, value) => {
    ...
    setCounts(prevCounts => ({
      ...prevCounts,
      [denomination]: newCount === 0 ? '' : newCount,
    }));
  };
  ...
  const handleSendToN8N = async () => {
    try {
      const response = await fetch("http://localhost:5678/webhook/efectivo", {
        method: "POST",
      ...
      if (response.ok) {
        alert("Datos de Efectivo enviados correctamente a n8n ✅");
      } else {
        alert("Error al enviar datos de Efectivo a n8n ❌");
      }
    } catch (error) {
  ...
  return (
    <div className="bg-white dark:bg-gray-800 p-6 md:p-8 rounded-xl shadow-lg">
      ...
      <div className="mt-8 flex justify-between items-center">
        <button
          onClick={handleReset}
          className="flex items-center justify-center gap-2 bg-red-500 hover:bg-red-600 text-white font-bold py-2 px-4 rounded-lg transition-colors"
        >
          <Trash2 size={18} />
          Limpiar Todo
        </button>
